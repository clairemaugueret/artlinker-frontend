...
export default function ArtScreen({ navigation, route }) {
  ...
  const [activeArtitemAllImagesSlide, setActiveArtitemAllImagesSlide] =
    useState(0); //slide actif du carrousel oeuvre principale
  const carouselItemsAuthorRef = useRef(null); //référence pour le carrousel des autres oeuvres de l'artiste
  const [activeItemsAuthorSlide, setActiveItemsAuthorSlide] = useState(0); //slide actif du carrousel autres oeuvres de l'artiste
  ...
  useEffect(() => {
    setActiveArtitemAllImagesSlide(0); // Réinitialiser le slide actif à 0 lors du changement de l'oeuvre principale
